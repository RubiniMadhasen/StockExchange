...
class DemoApplicationTests {
   ...
   /*@Test
    ...
    {   int orderId=1;
    	...
    	when(sortRepository.updateSellOrderByOrderId(orderId, shares)).thenReturn(1);
    	assertEquals(0,sortService.executeBuyTrade(noOfShares, companyName, sharePrice));
   ...
   @Test
    public void executeSellTrade()
    {
    	...
    	int sharePrice=1005;
    	int shares=25;
    	when(sortRepository.findAllBuyOrders()).thenReturn(Stream.of(new SortDTO(1,"wiley",20,1000),new SortDTO(2,"google",50,1500)).collect(Collectors.toList()));
    	when(sortRepository.updateBuyOrderByOrderId(orderId, noOfShares)).thenReturn(1);
    	assertEquals(0,sortService.executeSellTrade(noOfShares, companyName, sharePrice));
